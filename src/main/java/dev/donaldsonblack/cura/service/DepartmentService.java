...
public class DepartmentService {
	...
	public Department save(DepartmentCreateRequest req) {
		Department d = Department.builder().name(req.name()).location(req.location()).parent(req.parent()).build();
	...
	public Page<Department> findAllForUser(UUID sub, Pageable pageable) {
		return repo.findAllForUser(sub, pageable);
	}
