...
public class DepartmentController {
	...
	@Operation(summary = "List all departments", description = "Returns a paginated list of all departments.", tags = {"Department"})
	@GetMapping
	...
	@Operation(summary = "Get department by ID", description = "Fetch a single department by its ID.", tags = {"Department"})
	@GetMapping("/{id}")
	public ResponseEntity<Department> getById(@PathVariable Integer id) {
		return service.getById(id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
	}

	@Operation(summary = "Create new department", description = "Creates a new department from the provided request.", tags = {"Department"})
	@PostMapping
	...
	@Operation(summary = "List department members", description = "Returns the list of users assigned to a department.", tags = {"Department"})
	...
	@Operation(summary = "Update department (partial)", description = "Partially update fields of a department by ID.", tags = {"Department"})
	@PatchMapping("/{id}")
	public ResponseEntity<Department> put(@PathVariable Integer id, @Valid @RequestBody DepartmentUpdateRequest req) {
		return service.update(id, req).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
	}

	@Operation(summary = "Replace department", description = "Replaces an existing department or creates a new one if it doesn't exist.", tags = {"Department"})
	@PutMapping("/{id}")
	public ResponseEntity<Department> patch(@PathVariable Integer id, @Valid @RequestBody DepartmentCreateRequest req) {
